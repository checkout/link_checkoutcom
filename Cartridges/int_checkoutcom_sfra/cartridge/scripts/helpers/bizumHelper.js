...
/**
 * Helper for Bizum asynchronous webhook notification storage.
 *
 * Reuses the `webhookNotification` SFCC custom object type (primary key: orderNo)
 * to bridge the server-to-server CKO webhook with the client-side polling loop.
 * The same custom object type is shared with MB Way.
 *
...
var bizumHelper = {
    ...
    storeWebhookNotification: function(orderNo, hook) {
        try {
            ...
            Logger.error('bizumHelper.storeWebhookNotification failed for order {0}: {1}', orderNo, e.message);
        }
    },
    ...
    deleteWebhookNotification: function(orderNo) {
        try {
            ...
            Logger.error('bizumHelper.deleteWebhookNotification failed for order {0}: {1}', orderNo, e.message);
        }
    },
};

module.exports = bizumHelper;
